...
function renderCars(data) {
  ...
  <div class="car-card">
    <div class="img-container" style="position: relative;">
      ...
      <button class="fav-btn" onclick="toggleFavorite(event, '${item.id}')" id="fav-${item.id}">
...
async function toggleFavorite(event, carId) {
    ...
    if (!token) {
        Swal.fire('ავტორიზაცია', 'გთხოვთ გაიაროთ ავტორიზაცია', 'info');
        return;
    }
    ...
    try {
        const favUrl = `https://rentcar.stepprojects.ge/api/Users/${phone}/favorites/${carId}`;
        ...
        if (response.ok || response.status === 204) {
            ...
            Swal.fire({
                ...
                position: 'top-end',
                icon: 'success',
                title: btn.classList.contains('active') ? 'დაემატა ფავორიტებში' : 'წაიშალა ფავორიტებიდან',
                showConfirmButton: false,
                timer: 1500
            });
        } else {
            ...
            if (response.status === 404) {
                const retryUrl = `https://rentcar.stepprojects.ge/api/Users/${phone}/favorite-cars/${carId}`;
                const retryRes = await fetch(retryUrl, {
                    method: 'POST',
                    headers: { 'Authorization': `Bearer ${token}` }
                });
                if (retryRes.ok) btn.classList.toggle('active');
            }
        }
    } catch (error) {
